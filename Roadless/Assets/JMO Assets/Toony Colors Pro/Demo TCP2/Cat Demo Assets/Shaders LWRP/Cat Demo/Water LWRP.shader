...
// This shader has been written manually, the Water Template is not yet available for LWRP

Shader "Toony Colors Pro 2/Examples LWRP/Cat Demo/Water"
{
	Properties
	{
		[TCP2HeaderHelp(Base)]
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 Fragment(Varyings input) : SV_Target
			{
				...
				float sceneZ = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, input.sPos.xy / input.sPos.w), _ZBufferParams);
				float partZ = LinearEyeDepth(input.sPos.z / input.sPos.w, _ZBufferParams);
